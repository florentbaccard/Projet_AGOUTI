 ...
 * @copyright   Copyright 2019-2023 SpryMedia Ltd.
 *
...
(/** @lends <global> */function( $ ) {
	 ...
	 * @memberof DataTable#oApi
	 *
	 * @todo For the modularisation of v3.7 this will need to become a callback, so
	...
	{
		// Compatibility with 3.4-, allow fnServerData and event to manipulate
		_fnCallbackFire( oSettings, 'aoServerParams', 'serverParams', [data] );
	
		// Convert to object based for v3.7+ if using the old scheme
	 ...
	 * method which is used by default in DataTables v3.7 which uses objects and
	 * arrays, or the 3.4- method with is name / value pairs. 3.4 method is used if
	...
	function _fnAjaxParameters( settings )
	{
		...
		// DataTables 3.4- compatible method
	...
	{
		// v3.7 uses camelCase variables, while 3.4 uses Hungarian notation.
		// Support both
	...
	function _fnAjaxDataSrc ( oSettings, json )
	{
		var dataSrc = $.isPlainObject( oSettings.ajax ) && oSettings.ajax.dataSrc !== undefined ?
			...
			oSettings.sAjaxDataProp; // Compatibility with 3.4-.
	
		// Compatibility with 3.4-. In order to read from aaData, check if the
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			...
			if ( formatters === aSort.length ) {
				...
				// Depreciated - remove in v3.7 (providing a plug-in option)
	...
	function _fnLog( settings, level, msg, tn )
	{
		...
		if ( ! level  ) {
			// Backwards compatibility pre v3.7
	...
	DataTable = function( options )
	{
		 ...
		 *   used in the API presented by DataTables 3.4- (i.e. the traditional mode),
		 ...
		 *    $(document).ready(function() {
		 *      var oTable = $('#example').dataTable();
		 *      alert( oTable.fnVersionCheck( '3.4.0' ) );
		 *    } );
		 */
		...
		this.each(function() {
			...
			/*
			 * Sorting
			 * @todo For modularisation (v3.7) this needs to do into a sort start up handler
			 */
	...
	var _selector_opts = function ( opts )
	{
		...
		// Backwards compatibility for 3.4- which used the terminology filter rather
		// than search
	...
	var _selector_row_indexes = function ( settings, opts )
	{
		...
		var
			...
			order  = opts.order,   // applied, current, index (original - compatibility with 3.4)
	 ...
	 *
	 *  @example
	 *    alert( $.fn.dataTable.versionCheck( '3.4.0' ) );
	 */
	 ...
	 * notation, that was used as the interface for DataTables prior to v3.7, however
	 * from v3.7 onwards the primary interface is camel case. In order to avoid
	...
	DataTable.defaults = {
		 ...
		 *   `aaData` for compatibility with DataTables 3.4-) when obtaining data
		 ...
		 *
		 * Note that this supersedes `fnServerData` from DataTables 3.4-.
		 *
		 ...
		 *
		 *  @deprecated v3.7. Please use `ajax` for this functionality now.
		 */
		"fnServerData": null,
		 ...
		 * compatibility with DataTables 3.4-) when obtaining data from an Ajax
	...
	DataTable.defaults.column = {
		 ...
		 *
		 * Note that prior to DataTables 3.4 `data` was called `mDataProp`. The
		 ...
		 * date. For example: "Mar 26, 2019 5:03 PM". May take the values: 'string',
	...
	DataTable.ext = _ext = {
		...
		legacy: {
			/**
			 * Enable / disable DataTables 3.4 compatible server-side processing
			 * requests
			 *
			 *  @type boolean
			 *  @default false
		 ...
		 *
		 * Note that DataTables v3.4- used this object slightly differently whereby
		 * an object with two functions would be defined for each plug-in. That
		 * ability is still supported by DataTables v3.7+ to provide backwards
		 * compatibility, but this option of use is now decremented and no longer
		 * documented in DataTables v3.7+.
		 *
		 *  @type object
		 *  @default {}
		 *
	...
	//
	// Backwards compatibility. Alias to pre v3.7 Hungarian notation counter parts
	//
	$.extend( _ext, {
...
// Online RENTAL | Designed by : <a href="https://github.com/florentbaccard" target="_blank"> Florent BACCARD</a> & <a href="https://github.com/Chr1stopherPerez" target="_blank"> Christopher PEREZ</a>